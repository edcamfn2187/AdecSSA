...
export const FinancialManagement: React.FC = () => {
  ...
  const transactionsIncome = transactions.filter(t => t.type === 'INCOME').reduce((acc, t) => acc + toNumber(t.amount), 0);
  const tithesIncome = tithes.reduce((acc, t) => acc + toNumber(t.amount), 0);
  const totalIncome = transactionsIncome + tithesIncome;

  const totalExpense = transactions.filter(t => t.type === 'EXPENSE').reduce((acc, t) => acc + toNumber(t.amount), 0);
  const balance = totalIncome - totalExpense;

  const regionalStats = regionals.map(reg => {
    const regTrans = regionalTransactions.filter(t => t.regional_id === reg.id);
    const income = regTrans.reduce((acc, t) => acc + toNumber(t.income_amount), 0);
    const expense = regTrans.reduce((acc, t) => acc + toNumber(t.expense_amount), 0);
    return {
      ...reg,
      income,
      expense,
  ...
  tithes.forEach(t => {
    ...
    incomeByCategory[typeName] = (incomeByCategory[typeName] || 0) + toNumber(t.amount);
  });
